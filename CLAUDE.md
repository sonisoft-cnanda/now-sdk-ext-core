...
- Winston-based `Logger` class for structured logging. It is a thin named facade over
  ONE process-wide winston logger built by `src/util/LogConfig.ts` — not a logger per
  instance. That matters: `ServiceNowRequest` is constructed per HTTP call, so a
  per-instance logger opened four never-closed file streams per request.
- **File logging is off by default and writes nothing to the working directory.** The
  consuming application calls `configureLogging()` once at boot (or sets `NEX_LOG_FILE` /
  `NEX_LOG_DIR` / `NEX_LOG_LEVEL`); the default is stderr at `warn`. Never log to stdout
  from this library — the MCP server carries JSON-RPC there.
