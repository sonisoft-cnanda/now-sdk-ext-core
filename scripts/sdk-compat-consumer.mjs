...
async function runConsumerSmoke() {
    if (process.argv[2] === '--self-test=success') {
        await writeFile(join(output, 'self-test-marker'), 'created');
        return;
    }
    if (process.argv[2] === '--self-test=failure') {
        await writeFile(join(output, 'self-test-marker'), 'created');
        throw new Error('Forced compatibility fixture failure');
    }

    run('npm', ['run', 'build'], root);
    const pack = JSON.parse(run('npm', ['pack', '--json', '--pack-destination', output], root).stdout);
...
try {
    ...
    if (process.env.SDK_COMPAT_KEEP_TEMP === '1') {
        process.stderr.write(`Preserving SDK compatibility fixture for diagnosis: ${output}\n`);
    } else {
